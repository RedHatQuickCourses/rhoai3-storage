...
In the previous labs, we focused on **unstructured data** (images, models, logs) stored in Object Storage. However, real-world AI often requires **structured data**—customer records in PostgreSQL, sales transactions in Snowflake, or feature vectors in Redis.
...
This requires a two-step workflow:
 1.  **Define the Standard (The Template):** You create a `ConnectionType` resource that defines the schema (e.g., "I need a Host, a Port, and a Password").
 2.  **Provision the Access (The Instance):** You create the `Secret` that fills in those blanks.
