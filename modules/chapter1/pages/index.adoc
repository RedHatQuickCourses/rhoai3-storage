...
image::storage-connv2.png[align="center", title="Storage Types and Protocols AI Models for Generative AI"]
...
=== 2. The Persistence Tier (Dual-Storage Pipelines)
 * **The Challenge:** S3 Ping-Ponging and Data Duplication. How do you share massive datasets efficiently between pipeline tasks without losing your execution logs? 
 * **The Solution:** **The Hybrid Architecture.** OpenShift AI 3.3 pipelines require a dual-storage approach. We use **S3-compatible Object Storage** as the permanent archive for artifacts and logs, paired with high-speed **Persistent Volume Claims (PVCs)** for ephemeral pipeline workspaces.
 * **The Outcome:** **High-Speed Transit.** Pipeline components exchange massive datasets directly via the local cluster disk during execution, while the final results and logs are safely committed to the cloud archive.
...
In the upcoming modules, you will:
 ...
 1.  **Analyze the Physics:** Understand why downloading models to `emptyDir` kills production stability, and how to architect the Hybrid Pipeline storage model.
